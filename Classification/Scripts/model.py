...
mask = "False"
mmmodel = "Resnet18"
print("This is the 1) mask:", mask, "; 2) model: ",mmmodel)


# # データの読み込み
Data_02Nami_20=list(np.load("../imread_02Namix20_nomask.npy",allow_pickle=True))
Data_01_20=list(np.load("../imread_01x20_nomask.npy",allow_pickle=True))
Data_02Nami_40=list(np.load("../imread_02Namix40_nomask.npy",allow_pickle=True))
Data_01_40=list(np.load("../imread_01x40_nomask.npy",allow_pickle=True))

Data_02Nami=Data_02Nami_20
Data_01=Data_01_20

for x in Data_02Nami_40:
    if max(x.shape[0],x.shape[1])<=256:
        Data_02Nami.append(x)
for x in Data_01_40:
    if max(x.shape[0],x.shape[1])<=256:
        Data_01.append(x)

Data_02Nami=np.array(Data_02Nami)
Data_01=np.array(Data_01[:50000])
...
name="Resnet18_nomask_Output.jpg"
plt.savefig(name)
...
plt.plot(epochs, Accuracyvalid,'r',label='Validation accuracy')
plt.title('Training and Validation accuracy')
name_specify="Resnet18_nomask_Output_specify.jpg"
plt.savefig(name_specify)
print("Saved output as , ", name_specify, flush=True)
print("")

# save model
PATH = "Model_Resnet18_nomask.pkl"
